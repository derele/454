...
mf <- merge(mf, bm.diff[[2]][,c("id", "padj", "log2FoldChange")],
            by.x="Bm.hit", by.y="id", 
            all=TRUE, suffixes=c("",".Bm"))

mf <- merge(mf, ce.diff[[2]][,c("id", "padj", "log2FoldChange")],
            by.x="Ce.hit", by.y="id", 
            all=TRUE, suffixes=c("", ".Ce"))

the22 <- head(mf[order(mf$padj), ], n=22)
