...
\Sexpr{cn(length(eel.contigs))} contigs.
...
data, and \Sexpr{cn(sum(pre.screen.tab$eelmRNA))#$} reads removed. By
...
\Sexpr{cn(sum(pre.screen.tab$AcrRNA))#$} were tagged and removed. The
L2 larval library proved to have contributions for other cobionts of
the eel, and \Sexpr{cn(pre.screen.tab[["L2","Cercozoa"]])} reads were
...
\Sexpr{cn(nMN)} contigs were supported by both assembly algorithms,
\Sexpr{cn(nbad.c)} contigs were supported by only one of the assembly
algorithms and \Sexpr{cn(nSing)} singletons were not assembled by
...
Nearly one third (\Sexpr{cn(nallannotations)}) of the
\textit{A. crassus} TUGs were annotated with at least one identifier,
and \Sexpr{cn(all.identifier)} had GO, EC and KEGG annotations (Figure
1).
...
\Sexpr{round(u.test[["p.value"]],3)}; two sided Mann-Whitney-test).
...
non-lethal phenotypes (p=\Sexpr{round(u.rnai$p.value,3)#$}, two-sided
U-test).
...
differential expression using ideg6 \Sexpr{cn(nrow(ideg6))} for
...
enumerated adn the number of reads regarded valid afterthis screening
is enumerated and their span (in bases) is given.

Libraries and sequencing read from those are then caracterised
according to their mappin to the final assembly: The number of reads
...
align(trim_mark.tab) <- "p{4.5cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}"
...
Additional table 1 (a + b): All data computed on the contig level, (a)
including sequences (raw, coding, imputed, protein), or (b) listing
only the metadata.

Additional table 2: High quality SNPs. The contig, the base relative
...
Additional Figure 1: When SNPs in or adjacent to homopolymeric regions
...
Additional Figure 2: Overabundance of SNPs at codon-position two (a)
