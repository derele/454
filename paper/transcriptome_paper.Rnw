...
identified \Sexpr{n.fish} TUGs that had higher, significant similarity to
...
span \Sexpr{sum(nchar(contig.df[contig.df$Ac,"seq"]))#$} bases) that
...
trim_mark <- merge(trim_mark, colSums(counts[,3:8]), by.x="library", by.y="row.names")
names(trim_mark)[length(trim_mark)] <- "mapping.unique"

sum.Ac <- colSums(counts[counts$contig%in%contig.df[contig.df$Ac, "contig"], 3:8])
trim_mark <- merge(trim_mark, sum.Ac, by.x="library", by.y="row.names")
names(trim_mark)[length(trim_mark)] <- "mapping.Ac"

sum.Ac.MN <- colSums(counts[counts$contig%in%contig.df[contig.df$AcMN, "contig"], 3:8])
...
print(xtable(GO.over.dn.ds), tabular.environment="longtable",
      floating=FALSE,
      hline.after=c(0, 10, 20, 30), include.rownames=FALSE)
