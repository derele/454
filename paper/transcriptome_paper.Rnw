...
per-base unique coverage was \Sexpr{cov.u}. The mean per-base coverage
...
\Sexpr{n_suMN_Ac} quality-contigs, by using sequence labeled
...
these \Sexpr{round(total.cov8.nsyn, 2)} non-synonymous sites and
\Sexpr{round(total.cov8.syn, 2)} synonymous sites were covered at least 8-fold,
...
kilo-base was \Sexpr{round(perc.per.cov8.b,2)},
...
(\Sexpr{nrow(subset(contig.df,!is.na(dn.ds)&category%in%"MN"))} high
  ...
  \Sexpr{round(mean(contig.df$dn.ds, na.rm=TRUE), 2)#$}, as in the first the
...
predicted \Sexpr{nsigP} signal peptide cleavage sites and
\Sexpr{nsigPtm} signal peptides with a transmembrane signature.
...
\Sexpr{round(u.test.AC[["p.value"]], 3)}; one sided U-test; see also
...
adjacent to homopolymer regions from \Sexpr{read.dn.ds} to
