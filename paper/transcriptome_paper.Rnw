...
\begin{abstract}
  ...
  ecologically and evolutionary interesting nematode. It has been
  introduced from Asia, where it parasitises the Japanese eel
  \textit{Anguilla japonica}, to Europe 30 years ago, parasitising the
  endangered, commercially exploited European eel \textit{Anguilla
    anguilla}. Phylogenetics places \textit{A. crassus} at a key
  position for the emergence of parasitism, basal to one of the major
  clades of parasitic nematodes, but differences in teh parasite's
  pehnotype and host relations (natural versus novel host) are not
  understood yet.
  \paragraph*{Results:} 
...
A total of \Sexpr{cn(TRIM["total","raw_reads"])} raw sequencing reads were
...
\Sexpr{cn(sum(nchar(R$seq)))#$} bases). In the eel dataset from
\Sexpr{cn(length(eel.all))} raw reads \Sexpr{cn(length(eel.clean))} were
...
inferred, for \Sexpr{cn(rnai.contigs[["TRUE"]])} TUGs a lethal phenotype.
...
\Sexpr{round(raw.ts.tv[,3],3)} (\Sexpr{tsv.raw.orf[1,3]} outside,
\Sexpr{tsv.raw.orf[2,3]} inside ORFs) is in good agreement with the
