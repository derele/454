...
\begin{abstract}
  ...
  assembled into 11,372 contigs of a high confidence assembly (highCA,
  spanning 6.6 Mb) and 21,153 singletons and contigs of a lower
  confidence assembly (lowCA, spanning an additional 6.2 Mb). Roughly
  55\% of the highCA derived contigs were annotated with domain- or
  protein sequence similarity match-derived functional
...
previously unstudied species \cite{pmid20950480}. Here we report on
the generation of a reference transcriptome for \textit{A. crassus}
based on Roche 454 data, and explore patterns of gene expression and
...
population of L2 larvae (Table 1). For host contamination screening a
liver sample from an uninfected \textit{An. japonica} was also
processed. RNA was reverse transcribed and amplified into cDNA using
the MINT-cDNA synthesis kit (Evrogen, Moscow, Russia). Emulsion PCR
and library preparation were performed for each cDNA library according
to the manufacturer's protocols (Roche/454 Life Sciences), and
sequenced on a Roche 454 Genome Sequencer.
...
sample. For the L2 larval library, which had a larger number of
...
high credibility assembly (highCA) derived contigs. We recorded the
presence of a lethal RNAi phenotype in the \textit{C. elegans}
ortholog of each TUG using the biomart -interface \cite{pmid22083790}
to WormBase v. 220 through the R-package biomaRt \cite{pmid19617889}.
...
A total of \Sexpr{cn(TRIM["total","raw_reads"])} raw sequencing reads
were generated for \textit{A. crassus} (Table 1). These were trimmed
for base call quality, and filtered by length to give
\Sexpr{cn(TRIM["total","valid"])} high-quality reads (spanning
\Sexpr{cn(sum(nchar(R$seq)))#$} bases). From An. japonica liver RNA
\Sexpr{cn(length(eel.all))} raw reads were generated, and
\Sexpr{cn(length(eel.clean))} retained after basic quality
screening. These eel reads were assembled into
\Sexpr{cn(length(eel.contigs))} contigs. The \textit{A. crassus} reads
...
\Sexpr{cn(sum(pre.screen.tab$eelmRNA))#$} reads removed. By comparison
...
in preliminary analyses), \Sexpr{cn(sum(pre.screen.tab$AcrRNA))#$}
were tagged and removed. The L2 larval library proved to have
contributions for other cobionts of the eel, and
\Sexpr{cn(pre.screen.tab[["L2","Cercozoa"]])} reads were removed
...
load("../Supporting_information/Assembly.Rdata")
...
The remaining \Sexpr{cn(length(raw.fasta))} reads (spanning
\Sexpr{cn(nBases)} bases) were assembled using the combined assembler
strategy \cite{pmid20950480}, employing Roche 454 gsAssembler (also
known as Newbler; version 2.6) and MIRA (version 3.21)
\cite{miraEST}. In this coassembly, \Sexpr{cn(nMN)} contigs were
supported by both assembly algorithms, \Sexpr{cn(nbad.c)} contigs were
supported by only one of the assembly algorithms and \Sexpr{cn(nSing)}
singletons were not assembled by either program (Table 2). Contigs
supported by both assemblers were longer, and were more likely to have
a significant similarity to previously sequenced protein coding genes
than contigs assembled by only one of the algorithms, or the remaining
unassembled singletons. These constitute the highCA, while those with
evidence from only one assembler and the singletons are the low
lowCA. These datasets were the most parsimonious (having the smallest
...
was prioritised, while in the complete assembly (highCA plus lowCA)
completeness was prioritised. The 40,187 sequences (contig consensuses
and singletons) in the complete assembly are referred to as
...
and identified \Sexpr{cn(n.fish)} TUGs that had significant, higher
...
hits). Additionally \Sexpr{cn(other.king[["Fungi"]])} TUGs had best,
...
to the Wolbachia or related Bacteria known as symbionts of nematodes and
...
\Sexpr{cn(nrow(contig.df[contig.df$Ac,]))#$} TUGs, spanning
\Sexpr{cn(sum(nchar(as.character(contig.df[contig.df$Ac,"seq"]))))#$}
bases (of which \Sexpr{cn(nrow(contig.df[contig.df$AcMN,]))#$} are
highCA-derived, and span
\Sexpr{cn(sum(nchar(as.character(contig.df[contig.df$AcMN,"seq"]))))#$}
...
For
\Sexpr{cn(nrow(contig.df[contig.df$Ac&contig.df$method!="no-prediction",]))}
...
\Sexpr{cn(nrow(contig.df[contig.df$Ac&imputed$full.length,]))} TUGs,
while for \Sexpr{cn(nrow(contig.df[contig.df$Ac&imputed$full.5p,]))}
the 5' ends and for
\Sexpr{cn(nrow(contig.df[contig.df$Ac&imputed$full.3p,]))} the 3' ends
were complete. In \Sexpr{cn(same.same[["FALSE"]])} TUGs the corrected
sequence with the imputed ORF was slightly changed compared to the raw
sequence by insertions or deletions necessary to obtain a continuous
reading frame. Using BLAST we determined that
\Sexpr{cn(nrow(contig.df[contig.df$Ac&!is.na(contig.df$Ce.hit),]))}
had significant similarity to \textit{C. elegans} proteins,
\Sexpr{cn(nrow(contig.df[contig.df$Ac&!is.na(contig.df$Bm.hit),]))}
TUGs matched \textit{B. malayi} proteins, and
\Sexpr{cn(nrow(contig.df[contig.df$Ac&!is.na(contig.df$nempep.hit),]))}
...
\Sexpr{cn(nrow(contig.df[contig.df$Ac&!is.na(contig.df$nr.bit),]))}
TUGs with significant similarities. We used annot8r
\cite{schmid_annot8r:_2008} to assign GO terms to \Sexpr{cn(nGO)}
TUGs, EC numbers for \Sexpr{cn(nEC)} TUGs and KEGG pathway annotations
for \Sexpr{cn(nKEGG)} TUGs (Table 2). Additionally \Sexpr{cn(nIPR)}
highCA derived contigs were annotated with GO terms through
InterProScan \cite{pmid11590104}. Nearly one third
(\Sexpr{cn(nallannotations)}) of the \textit{A. crassus} TUGs were
annotated with at least one identifier, and \Sexpr{cn(all.identifier)}
had GO, EC and KEGG annotations (Figure 1).
...
(\Sexpr{cn(nsigP)} with signal peptide cleavage sites and
\Sexpr{cn(nsigPtm)} signal peptides with a transmembrane
...
\cite{pmid19542151} on the \Sexpr{cn(nCov8bases)} bases of TUGs that
...
identified (Table 4; Additional Figure 4). Within the molecular
...
Figure 4). Contigs identified as novel to Spirurina and novel in
...
\Sexpr{round(u.test[["p.value"]],3)}; two sided
...
\cite{pmid21653522}, and \Sexpr{cn(nrow(Rh))} informative sites (where
two alleles were found in at least one assured genotype at least in
one of the nematodes) were identified in \Sexpr{cn(geno.contigs)}
...
The genome-wide representativeness of these \Sexpr{cn(nrow(Rh))} SNP
...
reads overall were excluded. Thus \Sexpr{cn(nrow(ideg6))} TUGs were
analysed for differential expression using ideg6 for normalisation and
the statistic of Audic and Claverie \cite{pmid9331369} for detection
of differences. Of these TUGs, 54 showed expression predominantly in
the male library, 56 TUGs were more highly represented in the female
library, 56 TUGs were primarily expressed in the libraries from
Taiwan, and 22 TUGs were overrepresented in the European library.
...
terms (Table 6 and Additional Figure 4). The ribosomal proteins were
...
activity especially related to metabolism (Table 7; Additional Figure
4). TUGs annotated as acyltransferase were upregulated in the European
libraries. However, the expression patterns for other TUGs with
overrepresented terms connected to metabolism did not show concerted
up or down-regulation. Thus for the term ``steroid biosynthetic
process'', 2 TUGs were downregulated and 3 contigs upregulated in
European nematodes. No enrichment of of signal peptide positive TUGs,
of TUG conservation categories, or TUGs with \textit{C. elegans}
orthologs with lethal or non-lethal RNAi-phenotypes was
identified. Significantly elevated dn/ds was found for TUGs
differentially expressed in European versus Asian nematodes (Fisher's
exact test p=0.007; also both up- or down-regulated were
...
write.csv(contig.df, "../paper/A.crassus_transcriptome_contig_data.csv",
row.names=FALSE)

write.csv(VARqp[, c("contig", "base", "from", "to",
"nfrom", "nto", "perc", "inORF",
"inFRAME", "effect")], "../paper/A.crassus_transcriptome_screened_SNPs.csv",
row.names=FALSE)

names(express.sex.sig) <- c("E2", "T1", "T2", "M", "LogFC")
write.csv(express.sex.sig, "../paper/A.crassus_transcriptome_sexDE.csv",
row.names=TRUE)

names(express.eel.sig) <- c("E2", "T1", "T2", "LogFC")
write.csv(express.eel.sig, "../paper/A.crassus_transcriptome_originDE.csv",
row.names=TRUE)
...
screening of NGS transcrioptome data has been shown to be important
...
using InterProScan \cite{pmid11590104}. Comparison with the complete
...
distribution. The corrected ti/tv value of 1.93 (1.25 outside and
...
under positive selection, thirteen peptiases were under positive
...
TUGs. Those thirteen peptidases are deeply conserved, as they have
unique orghtologue pairs in \textit{B. malayi} and
...
nematode was the most highly inbred, while the nematode from the wild
\textit{An. japonica} in Taiwan was the most highly outbred.\\

While our experiment was not designed for identification of
...
with 454 transcriptome data (e.g. \cite{pmid20470405}). We can only
tentatively infer differential expression of a gene under different
conditions (sex, origin) based on identification of significantly
differential expression between libraries. Genes over-expressed in the
male \textit{A. crassus} included major sperm proteins [83], and,
surprisingly, a suite of ribosomal proteins. Collagen processing
enzymes were overexpressed in the female nematodes in line with
modulation of collagen synthesis in nematode larval development, and
the ovoviviparity of this species \cite{pmid10637627}. Acetyl-CoA
acetyltransferase was identified as overexpressed in European
nematodes compared to the Asian one. Acetyl-CoA acetyltransferases act
in fatty-acid-oxidation in peroxisomes and mitochondria
\cite{pmid4721607}. Together with a change in steroid metabolism and
the enrichment of mitochondrially localised enzymes these suggest
changes in the energy metabolism of \textit{A. crassus} from different
origins. Possible explanations could include a change to more or less
aerobic processes in nematodes in Europe due to their bigger size
and/or increased availability of nutrients.  TUGs overexpressed in the
female libraries showed elevated levels of dn/ds but genes
overexpressed in males had decreased levels of dn/ds. The first
finding is unexpected, as genes overexpressed in female libraries will
also include TUGs related to larval development (such as the collagen
modifying enzymes discussed above), and these larval transcripts in
turn are expected to be under purifying selection because of
pleiotropic effects of genes in early development
...
expressed remains puzzling. Some male-overexpressed TUGs, such as
that encoding major sperm protein, showed elevated dn/ds. It is
unlikely that correlation of differential expression with positive
selection results from mapping artifacts, as all the ribosomal protein
encoding TUGs identified overexpressed in males have
very low dn/ds.\\
...
UK Natural Environment Research Council (award ) and the UK Medical
Research Council (award ). We are grateful to Karim Gharbi for oversight of the
project within the GenePool. Sujai Kumar and Graham Thomas gave
...
\cite{blaxter_molecular_1998}. TUGs without any hit at a given
...
trim_mark$life.st <- c("adult female", "adult female", "L2 larvae",
                       "adult male", "adult female", "adult female")

trim_mark$source.p <- c("Europe Rhine", "Europe Poland", "Europe Rhine",
                       "Asia cultured", "Asia cutured", "Asia wild")
...
names(trim_mark) <- c("sequencing library",
                      "lifecycle stage",
                      "source population",
                      "raw reads",
                      "low quality reads",
                      "\\textit{A. crassus} rRNA reads",
                      "eel-host mRNA reads",
                      "eel-host rRNA reads",
                      "Cercozoa reads (rRNA)",
                      "valid reads",
                      ...
                      "reads mapping to \\textit{A. crassus}- contigs",
                      "reads mapping highCA contigs",
                      "reads mapping to contigs with count >32")

trim_mark[,4:15] <- lapply(trim_mark[,4:15], as.numeric)
total.trim <- colSums(trim_mark[,4:15])
total.trim <- c("total","","", total.trim)

trim_mark <- t(trim_mark)
trim_mark <- cbind(trim_mark, total.trim)
trim_mark.tab <- xtable(trim_mark)

align(trim_mark.tab) <- "p{3.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}"

print(trim_mark.tab, floating=FALSE, include.colnames=FALSE,
      hline.after=c(0,1, nrow(trim_mark)), sanitize.text.function = identity)
@
...
T2$remaining.span <- tapply(contig.df[contig.df$Ac, ]$seq,
                            contig.df[contig.df$Ac,]$category%in%"MN",
...
T2$"all TUGs" <- as.numeric(as.character(T2$lowCA))+as.numeric(as.character(T2$highCA))

T2["non.u.cov", "all TUGs"] <- cov.nu.total
T2["cov", "all TUGs"] <- cov.u.total

dig <- t(matrix(c(rep(0,24), rep(c(0, 3 , 3, 3), 2), rep(0,52)), nrow=4))

rownames(T2) <- c("total contigs",
                  "contigs hitting rRNA",
                  "contigs hitting eel-mRNA or Chordata",
                  "non-eukaryote contigs",
                  "contigs remaining",
                  "total span of remaining contigs (in bases)",
                  "non-unique mean base coverage of contigs",
                  "unique mean base coverage of contigs",
                  "protein predictions by BLAST similarity", 
                  "protein predictions by ESTscan",
                  "protein predictions by longest ORF",
                  "contigs without protein prediction",
                  "contigs with complete 3\' end",
                  "contig with complete 5\' end",
                  "full length contigs",
                  "contigs with GO- annotation",
                  "contigs with EC- annotation",
                  "contigs with KEGG- annotation",
                  "contigs with InerProScan- annotation",
                  "contigs with BLAST hit to nematode",
...
nov.tab <- as.data.frame(rbind(nov.tab, nov.tab.MN))

nov.tab$"contig set" <- c("all TUGs", "all TUGs", "highCA", "highCA")
nov.tab$cutoff <- c(50, 80, 50, 80)


print(xtable(nov.tab[, c(6, 7, 1:5)]), floating=FALSE, include.rownames=FALSE)
...
GO.over.dn.ds$category <- ""
GO.over.dn.ds[c(1, GO.over.dn.ds.lines["MF"]+1, 
                GO.over.dn.ds.lines["MF"]+GO.over.dn.ds.lines["BP"]+1),
              "category"] <- c("molecular function", "biological process", "cellular compartment")
GO.over.dn.ds <- GO.over.dn.ds[, c(ncol(GO.over.dn.ds), 1:ncol(GO.over.dn.ds)-1)]

names(GO.over.dn.ds)[4] <- "Number annotated"
names(GO.over.dn.ds)[5] <- "Number dn/ds > 0.5"

GO.over.dn.ds.tab <- xtable(GO.over.dn.ds)
align(GO.over.dn.ds.tab) <- "p{1.7cm}p{1.7cm}lp{4.5cm}p{1cm}p{1cm}ll"

print(GO.over.dn.ds.tab, tabular.environment="longtable",
      floating=FALSE,
      hline.after=c(0, c(GO.over.dn.ds.lines["MF"],
        GO.over.dn.ds.lines["MF"]+GO.over.dn.ds.lines["BP"],
        GO.over.dn.ds.lines["MF"]+GO.over.dn.ds.lines["BP"]+ GO.over.dn.ds.lines["CC"])),
...
names(het.table) <- c("relative heterozygosity", "internal relatedness",
                      "homozygosity by loci", "standardised heterozygosity",
                      "informative SNPs")

het.table.tab <- xtable(het.table, digits=c(1, 2, 2, 2, 2, 0))
align(het.table.tab) <- "p{2.5cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}"
...
GO.over.sex$category <- ""
GO.over.sex[c(1, GO.over.sex.lines["MF"]+1, 
                GO.over.sex.lines["MF"]+GO.over.sex.lines["BP"]+1),
              "category"] <- c("molecular function", "biological process", "cellular compartment")
GO.over.sex <- GO.over.sex[, c(ncol(GO.over.sex), 1:ncol(GO.over.sex)-1)]

names(GO.over.sex)[4] <- "Number annotated"
names(GO.over.sex)[5] <- "Number significant"

GO.over.sex.tab <- xtable(GO.over.sex)

align(GO.over.sex.tab) <- "p{1.7cm}p{1.7cm}lp{4.5cm}p{1cm}p{1cm}ll"

print(GO.over.sex.tab, tabular.environment="longtable",
      floating=FALSE, 
      hline.after=c(0, c(GO.over.sex.lines["MF"],
        GO.over.sex.lines["MF"]+GO.over.sex.lines["BP"],
        GO.over.sex.lines["MF"]+GO.over.sex.lines["BP"]+ GO.over.sex.lines["CC"])),
        include.rownames=FALSE)
@ 

\newpage

\subsection*{Table 7 - Over- representation of GO-terms differentially
  expressed between Taiwanese and European \textit{A. crassus}}

<<echo=FALSE, results=tex>>=

GO.over.eel$category <- ""
GO.over.eel[c(1, GO.over.eel.lines["MF"]+1, 
                GO.over.eel.lines["MF"]+GO.over.eel.lines["BP"]+1),
              "category"] <- c("molecular function", "biological process", "cellular compartment")
GO.over.eel <- GO.over.eel[, c(ncol(GO.over.eel), 1:ncol(GO.over.eel)-1)]

names(GO.over.eel)[4] <- "Number annotated"
names(GO.over.eel)[5] <- "Number significant"


GO.over.eel.tab <- xtable(GO.over.eel)

align(GO.over.eel.tab) <- "p{1.7cm}p{1.7cm}lp{4.5cm}p{1cm}p{1cm}ll"

print(GO.over.eel.tab, tabular.environment="longtable",
      floating=FALSE, 
      hline.after=c(0, c(GO.over.eel.lines["MF"],
        GO.over.eel.lines["MF"]+GO.over.eel.lines["BP"],
        GO.over.eel.lines["MF"]+GO.over.eel.lines["BP"]+ GO.over.eel.lines["CC"])),
include.rownames=FALSE)
...
\subsubsection*{Even more additional figures}
\textbf{Additional pile of figures (please vote whether/how to include
  these)}: Subgraphs of the GO ontology categories induced by the top
10 terms identified as enriched in different sets of genes. Boxes
...
annotated gene is given. Black arrows indicate an ``is-a''
