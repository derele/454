...
\Sexpr{cn(sum(nchar(R$seq)))#$} bases). From \textit{An. japonica}
liver RNA \Sexpr{cn(length(eel.all))} raw reads were generated, and
...
the 5' ends and for
\Sexpr{cn(nrow(contig.df[contig.df$Ac&!imputed$full.3p,]))} the 3'
ends were missing. In \Sexpr{cn(same.same[["FALSE"]])} TUGs the
...
\Sexpr{cn(nrow(ideg6))} TUGs were analysed for differential expression
...
needed <- !names(contig.df)%in%c("ontology", "cov.ontology")

write.csv(contig.df[,needed], "../paper/A.crassus_transcriptome_contig_data.csv",
row.names=FALSE)
...
write.single.genes.rev <- function(genes, folder) {
  sapply(genes,
         function (x) {
           s <- fullest.assembly[x]
           if (as.logical(as.character(contig.df[contig.df$contig%in%x, "mstrand"]))){
             s <- revcom(fullest.assembly[x])}
           write.sequence(s, paste(folder, x, ".fasta", sep=""))
         })
}

write.single.genes.rev(peptidases, "../snp_analysis/peptidases/")

names(express.sex.sig) <- c("E2", "T1", "T2", "M", "LogFC")
express.sex.sig <- merge(express.sex.sig,
                         contig.df[, c("contig", "nempep.annot")], by.x=0, by.y="contig")
express.sex.sig <- express.sex.sig[order(express.sex.sig$LogFC), ]
write.csv(express.sex.sig, "../paper/A.crassus_transcriptome_sexDE.csv",
row.names=TRUE)

top10.sex <- express.sex.sig[c(1:5, (nrow(express.sex.sig)-4):nrow(express.sex.sig)),
                         "Row.names"]

write.single.genes.rev(top10.sex, "../gene_expression/top10_sex/")

names(express.eel.sig) <- c("E2", "T1", "T2", "LogFC")
express.eel.sig <- merge(express.eel.sig,
                         contig.df[, c("contig", "nempep.annot")], by.x=0, by.y="contig")
express.eel.sig <- express.eel.sig[order(express.eel.sig$LogFC), ]
write.csv(express.eel.sig, "../paper/A.crassus_transcriptome_originDE.csv",
row.names=TRUE)

top10.eel <- express.eel.sig[c(1:5, (nrow(express.eel.sig)-4):nrow(express.eel.sig)),
                         "Row.names"]

write.single.genes.rev(top10.eel, "../gene_expression/top10_eel/")
...
trim_mark$geolocation <- c("49.0262N; 8.310556E", "53.751959N; 21.730957E", "49.0262N; 8.310556E",
                           "22.6418N; 120.4440E", "22.6418N; 120.4440E", "22.5074N; 120.4220E")

trim_mark[7,] <- c("Eel", "", "", "", "", length(eel.clean), sum(nchar(eel.clean)),
                   length(eel.all), length(eel.all)-length(eel.clean), "liver", "Taiwan", "")

trim_mark <- trim_mark[, c(1, 10, 11, 12, 8, 9, 2, 4, 5, 3, 6, 7 )]
names(trim_mark)[c(1, 5)] <- c("library", "raw.reads")
...
trim_mark[,5:16] <- lapply(trim_mark[,5:16], as.numeric)
total.trim <- colSums(trim_mark[,5:16])
...
T2.tab <- xtable(T2[,c(2,1,3)], digits=dig)
