...
includes \Sexpr{nrow(VARqp)} SNPs. We retained \Sexpr{per.base} SNPs
per kb of contig sequence, with \Sexpr{s.per.s.base} synonymous SNPs
per 1000 synonymous bases and \Sexpr{s.per.s.base} non-synonymous SNPs
per 1000 non-synonymous bases. A mean dn/ds of
\Sexpr{round(mean(dn.ds.df$dn.ds),3)#$} was calculated for the
\Sexpr{nrow(dn.ds.df)} TUGs (\Sexpr{nrow(dn.ds.df[dn.ds.df$AcMN,])#$}
highCA-derived contigs) containing at least one synonymous SNP.
Overall dn/ds over all TUGs was \Sexpr{round(get.dn.ds(VARqp),2)}.
...
comapred to a non-lethal phenotype \Sexpr{round(u.rnai$p.value,3)#$}.
...
reads \Sexpr{n454.map} mapped uniquely (with their best hit) to the
