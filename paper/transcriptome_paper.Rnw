...
sequence. Using BLAST we determined that
...
and novel in clade III (Fisher's exact test p<0.001 ; Figure 3). The
...
overrepresented term. The twelve high dn/ds TUGs annotated as
peptidases each had unique orthologs in \textit{C. elegans} and
...
terms (Table 6a and Additional Figure 4). The ribosomal proteins were
...
activity especially related to metabolism (Table 6b; Additional Figure
...
exact test p=0.007; also both up- or downregulated were
...
\cite{pmid15579698}. Using this we identified 46 TUGs that may be
under positive selection. Twelve of these TUGs were annotated as
peptidases, and the GO term peptidases was significantly
overrepresented in the set of positively selected TUGs. These twelve
peptidases are deeply conserved, as all have unique orghtologue pairs
in \textit{B. malayi} and \textit{C. elgans}. Peptidases have
previously been proposed to have acquired prominent roles in
host-parasite interactions, and an \textit{A. crassus} trypsin-like
proteinase may be utilised by the tissue-dwelling L3 stage to
penetrate host tissue and an aspartyl proteinase may be a blood meal
digestive enzyme in adults \cite{polzer_identification_1993}. The
twelve proteinases under positive selection could be targets of
adaptive immunity developed against \textit{A. crassus}
\cite{knopf_migratory_2008, knopf_vaccination_2008}, which is often
only elicited against some but not all larvae \cite{molnar_caps}.
...
in allele contribution introduced by transcriptomic data and the
...
our dataset. For males the decreased dn/ds may be explained by the by
the high number of ribosomal protein-encoding TUGs, which all show
very low levels of dn/ds. That these TUGs were found to be
differentially expressed remains puzzling. Some male- overexpressed
TUGs, such as that encoding major sperm protein, showed elevated
dn/ds. It is unlikely that correlation of differential expression with
positive selection results from mapping artifacts, as all the
ribosomal protein encoding TUGs identified overexpressed in males have
very low dn/ds.\\
...
and L2-larvae (L2) of \textit{A. crassus} from \textit{An. anguilla}
in Europe (E1 and E2) and one male and two female nematodes from
...
align(trim_mark.tab) <- "p{4.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}"

print(trim_mark.tab, floating=FALSE, include.colnames=FALSE,
      hline.after=c(0,1, ncol(trim_mark)), sanitize.text.function = identity)
...
rownames(nov.tab) <- c("50 all", "80 all", "50 highCA", "80 highCA" )
...
also Additional Figure 4.

<<echo=FALSE, results=tex>>=

GO.over.dn.ds.tab <- xtable(GO.over.dn.ds)

align(GO.over.dn.ds.tab) <- "llp{4.5cm}llll"
...
\Sexpr{cn(nrow(Rh))} SNPs infromative at the individual
...
(Term). For a graph of
incuced GO-terms see also Addional Figure 4.\\
...
align(GO.over.sex.tab) <- "llp{4.5cm}llll"
...
align(GO.over.eel.tab) <- "llp{4.5cm}llll"
...
\Sexpr{round(cov.lm.raw$adj.r.squared,3)#$}) and (d) absence of such a
...
\textbf{Additional Figure 4}: Subgraphs of the GO-ontology categories
