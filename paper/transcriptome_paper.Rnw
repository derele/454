...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \title{The transcriptome of \textit{Anguillicola crassus} enables
    new approaches to an alien parasite species}
...
\begin{abstract}
  ...
  position for the emergence of parasitism, basally derived from one
  of the major clades of parasitic nematodes.
  \paragraph*{Results:}
  ...
  research on this species. It furthermore has the potential to
  provide unique insights into the evolution of parasitism in the
...
We sampled \textit{A. crassus} from \textit{An. japonica} from
Kao-Ping river and an adjacent aquaculture in Taiwan as described in
\cite{heitlinger_massive_2009} and worms from \textit{An. anguilla} in
Sniardwy Lake, Poland (53.751959N, 21.730957E) and from the
Linkenheimer Altrhein, Germany (49.0262N, 8.310556E). After
determination of the sex we stored adult nematodes in RNA-later
(Quiagen, Hilden, Germany) until extraction of RNA. We extracted RNA
from individual adult male and female nematodes and from a population
of L2 larvae (Table 1). We reverse transcribed RNA and amplified it
into cDNA using the MINT-cDNA synthesis kit (Evrogen, Moscow,
Russia). For host contamination screening was also processed a
liver-sample from an uninfected \textit{An. japonica}. We performed
Emulsion PCR for each cDNA library according to the manufacturer’s
protocols (Roche/454 Life Sciences), and sequenced it on a Roche 454
Genome Sequencer FLX.

We sequenced all samples using the FLX Titanium chemistry, except for
...
We extracted raw sequences in fasta-format (with the corresponding
...
(with parameters -minscore 20 -minmatch 10). We used seqclean
\cite{tgicl_pertea} to identify and remove poly-A-tails, low quality,
repetitive and short (<100 base) sequences. We compared all reads to a
set of screening databases using BLAST (expect value cutoff E<1e-5,
low complexity filtering turned off: -F F). The databases used were
(a) a host sequence database comprising an assembly of the
...
for which an offspring term was already in the table and no additional
counts supported overrepresentation.
...
were generated for \textit{A. crassus} (Table 1). Raw sequencing reads
are archived under study-accession number SRP010313 in the NCBI
Sequence Read Archive (SRA; http://www.ncbi.nlm.nih.gov/Traces/sra)
\cite{pmid22140104}. These were trimmed for base call quality, and
filtered by length to give \Sexpr{cn(TRIM["total","valid"])}
high-quality reads (spanning \Sexpr{cn(sum(nchar(R$seq)))#$}
bases). In the eel dataset from \Sexpr{cn(length(eel.all))} raw reads
\Sexpr{cn(length(eel.clean))} were assembled after basic quality
screening.

We then screened the \textit{A. crassus} reads for contamination by
host (\Sexpr{cn(sum(pre.screen.tab$eelmRNA))#$} reads matched
previously sequenced eel genes or our own \textit{An. japonica} 454
transcriptome, which had been assembled into
\Sexpr{cn(length(eel.contigs))} mRNA
contigs. \Sexpr{cn(sum(pre.screen.tab$AcrRNA))#$} reads matched large
or small subunit nuclear or mitochondrial ribosomal RNA sequences of
\textit{A. crassus} (Table 1). In addition to fish mRNAs, we
identified (and removed) \Sexpr{cn(pre.screen.tab[["L2","Cercozoa"]])}
...
We assembled the remaining \Sexpr{cn(length(raw.fasta))} reads
(spanning \Sexpr{cn(nBases)} bases) using the combined assembler
strategy \cite{pmid20950480} and Roche 454 GSassembler (version 2.6)
and MIRA (version 3.21) \cite{miraEST}. From this we derived
\Sexpr{cn(nMN)} contigs that were supported by both assembly
algorithms, \Sexpr{cn(nbad.c)} contigs only supported by one of the
assembly algorithms and \Sexpr{cn(nSing)} singletons that were not
...
identified \Sexpr{cn(n.fish)} TUGs that had higher, significant
...
We found 1,153 TUGs mapping to Eukaryota outside of the kingdoms
...
Additionally we found \Sexpr{cn(other.king[["Fungi"]])} TUGs with hits
to fungi (e.g Ajellomycetaceae, 53 hits) and
\Sexpr{cn(other.king[["Viridiplantae"]])} TUGs with hits to plants.
...
thus has \Sexpr{cn(nrow(contig.df[contig.df$Ac,]))#$} TUGs, spanning
...
TUGs using prot4EST \cite{wasmuth_prot4est:_2004} (Table 2) and
obtained the full open reading frame in
\Sexpr{cn(nrow(contig.df[contig.df$Ac&imputed$full.length,]))} TUGs,
while while for
\Sexpr{cn(nrow(contig.df[contig.df$Ac&imputed$full.5p,]))} the 5’ end
and for \Sexpr{cn(nrow(contig.df[contig.df$Ac&imputed$full.3p,]))} the
3' end was complete. In \Sexpr{cn(same.same[["FALSE"]])} TUGs the
...
(GO) terms for \Sexpr{cn(nGO)} TUGs, Enzyme Commission (EC) numbers
for \Sexpr{cn(nEC)} TUGs and Kyoto Encyclopedia of Genes and Genomes
(KEGG) pathway annotations for \Sexpr{cn(nKEGG)} TUGs (Table
2). Additionally \Sexpr{cn(nIPR)} highCA derived contigs were
annotated with GO terms through InterProScan
\cite{pmid11590104}. Nearly one third (\Sexpr{cn(nallannotations)}) of
the \textit{A. crassus} TUGs were annotated with at least one
identifier, and \Sexpr{cn(all.identifier)} had GO, EC and KEGG
...
protein sequence using SignalP \cite{pmid21959131} and predicted
\Sexpr{cn(nsigP)} signal peptide cleavage sites and
...
(\Sexpr{cn(nsigP.Bm)} signal peptide cleavage sites and
\Sexpr{cn(nsigPtm.Bm)} with transmembrane anchor) than for the
proteome of \textit{C. elegans} (\Sexpr{cn(nsigP.Ce)} signal peptide
cleavage sites and \Sexpr{cn(nsigPtm.Ce)} with transmembrane anchor).

We inferred the presence of a lethal RNAi phenotype in the orthologous
annotation of \textit{C. elegans}. For
\Sexpr{cn(rnai.contigs[["FALSE"]])} TUGs a non-lethal phenotype was
inferred, for \Sexpr{cn(rnai.contigs[["TRUE"]])} TUGs a lethal
phenotype.

\subsection*{Evolutionary conservation}

We classified \textit{A. crassus} TUGs as conserved, conserved in
...
and 80, respectively. Roughly half or 3/5 of the these highCA contigs
were identified as novel in \textit{A. crassus}.
...
\Sexpr{cn(nCov8bases)} bases of the TUGs that had coverage of more
...
\Sexpr{cn(nrow(VAR))} SNPs. The ratio of transitions (ti;
\Sexpr{cn(raw.ts.tv[["transitions"]])}) to transversion (tv;
\Sexpr{cn(raw.ts.tv[["transversions"]])}) in this set was
\Sexpr{round(raw.ts.tv[["ratioTS.TV"]],2)}. Using the prot4EST
predictions and the corrected sequences, \Sexpr{cn(nrawOrf[["TRUE"]])}
of the SNPs were predicted to be inside an ORF, with
\Sexpr{cn(nrawFrame[[1]])} at codon first positions,
\Sexpr{cn(nrawFrame[[2]])} at second positions and
\Sexpr{cn(nrawFrame[[3]])} at third positions. As expected ti/tv
inside ORFs (\Sexpr{tsv.raw.orf[2,3]}) was higher than outside ORFs
...
dataset includes \Sexpr{cn(nrow(VARqp))} SNPs. We retained
...
calculated for the \Sexpr{cn(nrow(dn.ds.df))} TUGs
(\Sexpr{cn(nrow(dn.ds.df[dn.ds.df$AcMN,]))#$} highCA-derived contigs)
...
sequencing libraries). This resulted in \Sexpr{cn(nrow(Rh))}
informative sites in \Sexpr{cn(geno.contigs)} contigs, where two
...
tests confirmed the representativeness of the \Sexpr{cn(nrow(Rh))}
...
\Sexpr{cn(length(raw.fasta))} reads \Sexpr{cn(n454.map)}
...
library in Table 1. To get unbiased estimates of expression we removed
also all contigs with a coverage lower than 32 reads overall and thus
analysed \Sexpr{cn(nrow(ideg6))} contigs using ideg6
\cite{pmid12429865} for normalisation and the statistic of Audic and
Claverie \cite{pmid9331369} for detection of differences.

\Sexpr{cn(nrow(express.eel.sig[express.sex.sig$LogFC<0,]))#$} contigs
showed an expression predominantly in the male library,
\Sexpr{cn(nrow(express.eel.sig[express.eel.sig$LogFC>0,]))#$} contigs
in the female
library. \Sexpr{cn(nrow(express.eel.sig[express.eel.sig$LogFC>0,]))#$}
...
Overrepresentation of GO-terms differentially expressed between
libraries from worms of European and Asian origin highlighted
catalytic activity especially related to metabolism (Table 6b;
Additional Figures 1). Acyltransferase contigs were all upregulated in
the European libraries. However, the expression patterns for other
contigs connected to metabolism did not show concerted up or
down-regulation (eg. for ``steroid biosynthetic process'' 2 contigs
were downregulated in the European library, 3 contigs upregulated).
...
We found ignificantly elevated dn/ds for contigs differentially
...
association with its host, we have used exhaustive filtering in
...
\cite{parkinson_transcriptomic_2004, wasmuth_extent_2008}. Furthermore
the basal position of \textit{A. crassus} in clade III could be
leading to most novelty in the clade not being shared with
\textit{A. crassus}.
...
Taiwanese and European origin and identified global SNPs, subsequently
we genotyped single worms for these SNPs. Screening of SNPs in or
adjacent to homopolymer regions improved overall measurements of SNP
quality. The ratio of transitions to transversions (ti/tv)
increased. Such an increase is explained by the removal of “noise”
associated with common homopolymer errors \cite{pmid21685085}. The
value of \Sexpr{round(raw.ts.tv[,3],3)} (\Sexpr{tsv.raw.orf[1,3]}
outside, \Sexpr{tsv.raw.orf[2,3]} inside ORFs) is in good agreement
with the overall ti/tv of humans (2.16 \cite{pmid21169219}) or
...
homopolymer regions from \Sexpr{raw.dn.ds} to
\Sexpr{round(mean(dn.ds.df$dn.ds),3)#$} after full screening. The most
...
final estimate of \Sexpr{round(mean(dn.ds.df$dn.ds),3)#$} seems
plausible: in transcripts from the female reproductive tract of
\textit{Drosophila} dn/ds was 0.15 \cite{pmid15579698} and 0.21 in the
male reproductive tract \cite{pmid11404480} (although for ESTs
specific to the male accessory gland were shown to have a higher dn/ds
of 0.47). A pyrosequencing study in the parasitic nematode
\textit{Ancylostoma canium} \cite{pmid20470405} reported dn/ds of 0.3.
...
\cite{pmid6449605}. We thus identified over-represented of GO-terms In
a set of contigs with dn/ds above this threshold. An enrichment in the
category ``peptidase acitivity'' highlighted twelve conitgs under
positive selection (from 43 with this annotation and a dn/ds
obtained). All twelve have different orthologs in \textit{B. malayi}
...
\Sexpr{cn(nrow(Rh))} SNPs with highest credibility and a high
information content for population-genetic studies. The low number of
SNPs inferred with assured genotypes reflects both the stringency of
software more targeted at even higher throughput genotyping (VCFtools
is rather designed for genomic data from the solexa platform
\cite{pmid21653522x}) and the (in comparison to genomic data)
additional variance in allele contribution introduced by the sampling
process of transcripts involved in the generation of transcriptomic
data.
...
from our study can be regarded as highly inbred, the worm from a
cultured \textit{An. japonica} in Taiwan as highly outbred.

We employed methods developed for the comparison of cDNA-libraries to
make inference about possible differential gene-expression according
to experimental groups (origin of sequencing-libraries)
\cite{pmid9331369}. Such approaches are widely used with
pyrosequencing-data (e.g. \cite{pmid20470405}). Nevertheless for the
statistically valid comparison of conditions, the unit of replication
would be the individual library and approaches respecting this fact
would be desirable. However, we were not able to use the R-packages
DESeq \cite{pmid20979621} or edgeR \cite{pmid19910308} developed for
count data from deep sequencing (but more targeted towards RNA-seq on
the solexa-platform) as both repetition and throughput of our present
...
proteins in the male library. That collagen processing enzymes are
overexpressed in female worms, filled with developing embryos and
larvae, is in line with a complicated regulation and modulation of
collagen in nematode larval development \cite{pmid10637627}.
...
showing hallmarks of positive selection \cite{pmid15795858,
  pmid11404480}. In \textit{Ancylostoma caninum} however,
...
similar enrichment of positively selected parasitism related
transcripts in our dataset. For males the decreased dn/ds can be
explained by the by the high number of ribosomal proteins, which are
all show very low levels of dn/ds, while single transcripts
(e.g. major sperm protein ;expressed in the male library only) showed
elevated dn/ds but did not level the overall effect. Given the
contrasting finding of lower dn/ds in male overexpressed genes it is
unlikely that correlation of differential expression with positive
selection results from mapping artifacts.

Genes differentially expressed according to worm-origin (in either
...
these factors are expected overlap and interact with differences in
male vs. female worms and the careful cross-examination of the above
factors with worm-sex is advised.
...
trim_mark$life.st <- c("adult f", "adult f", "L2 larvae",
...
print(trim_mark.tab, floating=FALSE, include.colnames=FALSE,
      hline.after=c(0,1, ncol(trim_mark)))
@   
...
Genotyping for a set of \Sexpr{cn(nrow(Rh))} SNPs, different
