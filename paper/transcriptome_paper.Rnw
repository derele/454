...
\Sexpr{sum(nchar(R$seq))#$} bases). In the eel data-set from
...
(\Sexpr{con.rnai["conserved","proportion"]}\%) than for othologs of
...
prtedictions and the corrected sequences, \Sexpr{nrawOrf[["TRUE"]]} of
the SNPs were predicted to be inside an ORF, with
\Sexpr{nrawFrame[[1]]} at codon first positons, \Sexpr{nrawFrame[[2]]}
...
\Sexpr{s.per.s.base} non-synonymous SNPs per 1000 non-synonymous
...
tests confirmed the usability of the \Sexpr{nrow(Rh)} SNP-markers for
...
mapping. Of the {length(raw.fasta)} reads \Sexpr{n454.map}
...
\Sexpr{nrow(contig.df[contig.df$Ce.hit%in%"C44B11.3",])#$} and \Sexpr{nrow(contig.df[contig.df$Bm.hit%in%"UniRef100_A8PYK7",])#$}
  ...
  \Sexpr{nrow(contig.df[contig.df$Ce.hit%in%"F58F6.1", ])#$} contigs.
    ...
    \Sexpr{nrow(subset(eel,padj.x<0.21&eel$padj.x>0.19))#$} contigs
...
\cite{pmid20478048,pmid20470405}, the combined assembly approach
generated a smaller number of contigs that had lower redundancy and
higher completeness. Projects using the mira assembler often report
...
expression in one sample can't validate underexpression.
...
\Sexpr{round(cov.lm.raw$adj.r.squared,3)#$} and afterwards
($R^2<$0.001,
$p=$\Sexpr{round(cov.lm$coefficients[2,"Pr(>|t|)"],3)#$} (d).
