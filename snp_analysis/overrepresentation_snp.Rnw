...
sigP.50.p <- ggplot(subset(contig.df, contig.df$Ac & !is.na(novel.50) & !is.na(sigP)),
  ...
  geom_bar(position="fill", color="black") + coord_polar(theta="y") +
  scale_y_continuous("") +
  scale_x_discrete("", labels=good.lab,
                   breaks=levels(contig.df$novel.50)) +
  scale_fill_manual(values=c("white", "darkblue","red")) +
  theme_bw() + 
  opts(title="proportion of TUGs in SignalP category\nby evolutionary conservation categories at bitscore threshold of 50", plot.background = theme_blank(), legend.position=c(.9, .9)) 
...
sigP.80.p <- ggplot(subset(contig.df, contig.df$Ac & !is.na(novel.80) & !is.na(sigP)),
  ...
  scale_x_discrete("", labels=good.lab,
                   breaks=levels(contig.df$novel.50)) +
  opts(title="proportion of TUGs in SignalP category\nby evolutionary conservation categories at bitscore threshold of 80", plot.background = theme_blank()) + 
  theme_bw()
...
sigP.50.p.MN <- ggplot(subset(contig.df, contig.df$AcMN & !is.na(novel.50) & !is.na(sigP)),
  ...
  scale_x_discrete("", labels=good.lab,
                   breaks=levels(contig.df$novel.50)) +
  scale_fill_manual(values=c("white", "darkblue","red"), guide="none") +
  opts(title="proportion of highCA contigs in SignalP category\nby evolutionary conservation categories at bitscore threshold of 50", plot.background = theme_blank()) +
  theme_bw()
...
vp1 <- viewport(width = 0.54, height = 0.5, x = 0.25, y = 0.24)
vp2 <- viewport(width = 0.54, height = 0.5, x = 0.25, y = 0.72)
vp3 <- viewport(width = 0.54, height = 0.5, x = 0.74, y = 0.24)
vp4 <- viewport(width = 0.54, height = 0.5, x = 0.74, y = 0.72) 
