...
TOGO.all.onto.dn.ds <- function (o, gl) {
  g <- factor(as.integer(geneN %in% gl))
  names(g) <- geneN
...
dn.ds.over <-   lapply(ont, function (on) TOGO.all.onto.dn.ds (on, big05))

GO.over.dn.ds <-   do.call("rbind", lapply(1:3, function (i) {
  GenTable(dn.ds.over[[i]][[1]], classic = dn.ds.over[[i]][[2]], 
