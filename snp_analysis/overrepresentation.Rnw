...
dn.ds.df.AC$sigP <- as.factor(dn.ds.df.AC$sigP)

## this for the easy nn-data
u.test <- wilcox.test(dn.ds ~ sigP, data=dn.ds.df)
u.test.AC <- wilcox.test(dn.ds ~ sigP, data=dn.ds.df.AC)
...
sigP.box <- ggplot(dn.ds.df.AC, aes(dn.ds.df.AC$sigP, dn.ds.df.AC$dn.ds)) + 
  geom_boxplot() + 
  scale_y_log10("dn/ds") +
  scale_x_discrete("", 
                   breaks=levels(dn.ds.df.AC$sigP),
...
dn.ds.df.AC$novel.50 <- as.factor(dn.ds.df.AC$novel.50)
dn.ds.df.AC$novel.80 <- as.factor(dn.ds.df.AC$novel.80)

novel.50.ndwd <- get.ndwd(dn.ds.df.AC, dn.ds.df.AC$dn.ds, dn.ds.df.AC$novel.50)
novel.50.kw.ph <- pvalue(novel.50.ndwd, method = "single-step")

novel.80.ndwd <- get.ndwd(dn.ds.df.AC, dn.ds.df.AC$dn.ds, dn.ds.df.AC$novel.80)
novel.80.kw.ph <- pvalue(novel.80.ndwd, method = "single-step")

u.novel.50 <- wilcox.test(dn.ds.df.AC$dn.ds~
                          as.factor(dn.ds.df.AC$novel.50%in%"novel.in.clade3"), alternative="less")

u.novel.80 <- wilcox.test(dn.ds.df.AC$dn.ds~
                          as.factor(dn.ds.df.AC$novel.80%in%"novel.in.clade3"), alternative="less")
@ 

<<novel.nn, echo=FALSE, results=hide>>=

sigP.50 <- table(contig.df$novel.50=="novel.in.nematoda", grepl("Yes*", contig.df$sigP))
sigP.80 <- table(contig.df$novel.80=="novel.in.nematoda", grepl("Yes*", contig.df$sigP))

sigP.50.p <- ggplot(subset(contig.df, !is.na(novel.50) & !is.na(sigP)),
...
sigP.80.p <- ggplot(subset(contig.df, !is.na(novel.80) & !is.na(sigP)),
...
png("../figures/signal_novel.png", width=2000, height=1000, res=144)
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 2)))
vplayout <- function(x, y)
    viewport(layout.pos.row = x, layout.pos.col = y)
...
# Make each plot, in the correct location
print(sigP.50.p, vp = vplayout(1, 1 ))
print(sigP.80.p, vp = vplayout(1, 2 ))

dev.off() 

## ## wtf this could be really, really bad, is it?
## summary(dn.ds~cov8.nsyn.sites+cov8.syn.sites, data=dn.ds.df.AC)
## summary(dn.ds~nsyn.sites+syn.sites, data=dn.ds.df.AC)
