...
c.p.SndO.plot <- ggplot(contig.df,
        ...
        geom_jitter(size=1) + 
        scale_y_log10("number of newbler contigs") +
        scale_x_log10("number of mira contigs") +
        theme_bw()

r.p.SndO.plot <- ggplot(contig.df,
                        aes(x=reads_through_Newbler, y=reads_through_Mira)) +
       geom_point(size=1) +
       scale_x_log10("reads trough newber") +
       scale_y_log10("reads trough mira") +
       theme_bw()
@ 
...
text.base <- ggplot(suj.sum, aes(x=bases, y=base_cov, label=assembly)) +
             ...
             opts(strip.text.y = theme_text(face="italic")) +
             theme_bw()
...
## italic strip does not work somehow !!!
