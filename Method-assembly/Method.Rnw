...
increased in this step form the \Sexpr{length(wrongNewblerSing)}
reported to \Sexpr{length(NewblerSing)}. We later also address the
...
n25.dist <- ggplot(MERGED, aes(x=new.split, y=..count..+1)) +
  geom_bar(binwidth=2, color="white") +
  scale_y_log10("number of occurances") +
...
the \texttt{mira}-singletons. \Sexpr{length(frass)} reads fell in this
category. 
...
Bm.all <- covred.common.blast("/home/ele/Data/454/assemblies/blasts/ALL.fasta_vs_Bm.blt", "/home/ele/db/blastdb/uniref100/uniref100_bm.fasta")
Bm.all <- cbind(Bm.all, db="B. malayi")

Ce.all <- covred.common.blast("/home/ele/Data/454/assemblies/blasts/ALL.fasta_vs_Ce.blt", "/home/ele/db/blastdb/wormpep220/wormpep220.fasta")
...
text.base <- ggplot(suj.sum, aes(x=bases, y=base_cov, label=assembly)) +
             ...
             scale_y_continuous("percent refference-bases covered") +
...
text.prot <- ggplot(suj.sum, aes(x=bases, y=prot_cov, label=assembly)) +
             ...
             geom_point() +
             facet_wrap(~species, scales="free_y", nrow=2) +
             scale_y_continuous("percent reffereence-proteins hit") +
             scale_x_continuous("bases in assembly")
...
text.prot80 <- ggplot(suj.sum, aes(x=bases, y=prot_cov80p, label=assembly)) +
               ...
               geom_point() +
               facet_wrap(~species, scales="free_y", nrow=2) +
               scale_y_continuous("percent refference-proteins covered > 80% length") +
               scale_x_continuous("bases in assembly")
...
<<blasts, echo=FALSE, cache=TRUE>>=

CEblast <- read.delim("/home/ele/Data/454/assemblies/blasts/ALL.fasta_vs_Ce.blt", header=FALSE)
BMblast <- read.delim("/home/ele/Data/454/assemblies/blasts/ALL.fasta_vs_Bm.blt", header=FALSE)

CEblast <- CEblast[!duplicated(CEblast$V1),]
BMblast <- BMblast[!duplicated(BMblast$V1),]

## isoforms?
Ce <- as.data.frame(cbind(contig=as.character(CEblast[,1]),
                          Ce.hit=as.character(CEblast[,2]),
                          Ce.bit=as.numeric(CEblast[,12])
))

Bm <- as.data.frame(cbind(contig=as.character(BMblast[,1]),
                          Bm.hit=as.character(BMblast[,2]),
                          Bm.bit=as.numeric(BMblast[,12])
))

contig.df <- merge(contig.df, Ce, all.x=TRUE)
contig.df <- merge(contig.df, Bm, all.x=TRUE)
...
show <- head(contig.df[contig.df$cluster.size>12,c(1, 3:13, 18, 19) ], n=2)
show <- rbind(show,
              head(contig.df[contig.df$cluster.size>1 &
                             contig.df$category=="M_n" ,c(1, 3:13, 18, 19) ], n=1))
show <- rbind(show,
              head(contig.df[contig.df$cluster.size>1 &
                             contig.df$category=="N_n" ,c(1, 3:13, 18, 19) ], n=1))

row.names(show) <- show$contig
show$contig <- NULL
xtable(t(show), caption="example table for assembly-measurements on contigs (as given in
                         \texttt{contig-data.csv)}", label="tab:ex-me")
@ 
...
Thereby we reduced our dataset to \Sexpr{nrow(contig.df)} tentative
unique genes (TUGs), redifining the ``fullest assembly'' dataset.


<<save, echo=FALSE>>=
## save what is needed downstream
save(file="Method.Rdata", list=c("contig.df", "con.pile", "con.pile.uniq", "per.con", "per.con.uniq"))
