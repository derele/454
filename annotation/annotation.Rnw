...
png("/home/ele/thesis/454/figures/Figure_1.png", width=400, height=600)
...
summary.factor(contig.df[contig.df$Ac & 
!contig.df$contig%in%Allannotations, "contig"]%in%contig.df[contig.df$Ac & (!is.na(contig.df$Bm.hit) | !is.na(contig.df$Ce.hit) | !is.na(contig.df$nempep.hit) | !is.na(contig.df$nr.bit)),"contig"])
@ 
